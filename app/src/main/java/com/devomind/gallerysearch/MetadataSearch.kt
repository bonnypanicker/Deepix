...
internal object MetadataSearch {
    ...
    data class Document(
        ...
        fun scoreTerm(term: String): Float {
            val dimensionText = "${width}x${height}"
            return maxOf(
        ...
        companion object {
            fun from(item: GalleryRepository.MediaItem, locale: Locale): Document {
    ...
    class Index internal constructor(private val documents: List<Document>) {
        fun search(query: String, allowedUris: Set<String>? = null): List<Hit> {
            val parsed = ParsedMetadataQuery.from(query) ?: return emptyList()
            val dateByUri = documents.associate { it.uri to it.dateMillis }
            return documents.asSequence()
                .filter { allowedUris == null || it.uri in allowedUris }
                .mapNotNull { document ->
                    val score = parsed.score(document) ?: return@mapNotNull null
                    Hit(uri = document.uri, score = score)
                }
                .sortedWith(
                    compareByDescending<Hit> { it.score }
                        .thenByDescending { hit -> dateByUri[hit.uri] ?: 0L }
                )
                .toList()
        }
    }
    ...
    fun buildIndex(items: List<GalleryRepository.MediaItem>): Index {
        ...
        return Index(items.map { item -> Document.from(item, locale) })
    }

    fun buildDocuments(items: List<GalleryRepository.MediaItem>, locale: Locale = Locale.getDefault()): List<Document> {
        return items.map { item -> Document.from(item, locale) }
    }

    fun indexFromDocuments(documents: Collection<Document>): Index {
        return Index(documents.toList())
    }

    fun search(query: String, items: List<GalleryRepository.MediaItem>): List<Hit> {
    ...
    private data class ParsedMetadataQuery(
        ...
        fun score(document: Document): Float? {
            if (regex != null && !regex.containsMatchIn(document.searchableText)) return null
            if (fieldFilters.any { !it.matches(document) }) return null

            var total = if (regex != null) 6f else 0f
            for (term in textTerms) {
                val termScore = document.scoreTerm(term)
    ...
    private data class FieldFilter(
        ...
        fun matches(document: Document): Boolean {
            return when (field) {
                "ext" -> operator == ComparisonOperator.Equals && document.extension == value
                "id" -> compareInt(document.id.toIntOrNull())
                "year" -> compareInt(document.year)
                "month" -> compareInt(document.month)
                "day" -> compareInt(document.day)
                "width" -> compareInt(document.width)
                "height" -> compareInt(document.height)
                else -> false
            }
        }
