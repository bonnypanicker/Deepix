...
3. **Branding.** Finalize the launcher icon (proper adaptive icon: foreground/background layers, monochrome layer for themed icons on API 33+), themed splash icon, and consistent in-app "Deepix" naming (drawer header, Settings About).
4. **Versioning & hygiene.** Bump `versionCode`/`versionName` scheme (semantic + CI-incremented code); confirm `detekt` passes clean; add a `CHANGELOG.md`.
